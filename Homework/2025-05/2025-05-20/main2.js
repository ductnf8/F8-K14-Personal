...
const handleButtonClick = (value, screenE) => {
    ...
    // Ngăn lỗi cú pháp: Chỉ chặn **, */, //; cho phép +-, /-, /+, v.v.
    const lastChar = currentExpression.slice(-1);           // Lấy ký tự cuối
    const invalidOperatorPairs = ['**', '*/', '//'];       // Các cặp toán tử không hợp lệ
    const operators = ['+', '-', '*', '/'];                 // Danh sách toán tử
    if (operators.includes(value) && operators.includes(lastChar)) {
        const pair = lastChar + value;                       // Tạo cặp toán tử
